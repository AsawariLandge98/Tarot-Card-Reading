...
def inquiry_reply(request, pk):
    ...
    if obj.email:
        def send_reply():
            try:
                send_mail(
                    subject='Re: Your Inquiry — Astro Annie',
                    message=f"""Namaste {obj.name} ji,

Thank you for reaching out to Astro Annie. 🙏

We have received your message and Annie ji will get back to you shortly.

Your message:
"{obj.message}"

For urgent queries, WhatsApp directly at +91 81789 73198.
...
""",
    ...
    return Response({'message': 'Replied and email sent.'})
